...
MAX_CHAIN_TURNS = 20
VIRTUAL_USER_PROMPT_VERSION = "v6-human-shortchat-5"
...
def move_instruction(move: str, persona: dict) -> str:
    ...
    guides = {
        "open": (
            f"围绕「{topic}」随便开口。像刚打开对话框：半句、犹豫、只抛一个模糊点。"
            "禁止自我介绍完整背景；禁止一次说清要什么、为什么、约束是什么。"
            "开口不要自己把类比/解释讲完（别写成“是不是可以理解成A，其实是B”那种长疑问）。"
            "更像真人：例如“最近这方向有点乱”“你知道…吗”，让小埋来追问。"
            "不要在第一句里同时提到项目背景和你想要的结果。"
        ),
        ...
        "narrow": "只收窄到一个更小的点；一句话问完。不要用“先别管…我就想知道…”这种模板句。",
        "ask_detail": "只追问一个具体点（方法/数据/评测其一），一问一答式，不要复述对方长文；别下“直接告诉我/给推荐语”这类交付指令。",
        "ask_evidence": "随口质疑有没有读全文、链接靠不靠谱，别写成审稿意见。",
        "challenge": "觉得结论可能吹大了，用口语顶一句，不要长篇反驳。",
        "ask_code": "更关心有没有代码/仓库，随口问问就行，别写选型标准。",
        "ask_shorter": "嫌长：让对方短一点、先说重点；可以不耐烦，但别列格式要求。不要写成“先别…我就想知道…”模板。",
    }
...
2. 像微信打字：尽量 1 句，最多 2 短句；总长度宁短勿长。开口尤其要短，别一次把类比讲完。
3. 不要复述小埋刚说的长段落；不要“等下…其实…算了…我就想知道…”一串连环铺垫。
4. 不要用“先别/先别管/先别贴……我就想知道……”这种模板句，直接问那一个点。
5. 不要把背景+目标+约束+交付物一次讲全，宁可让小埋追问。
6. 禁止复述人设里的背景句/目标句；不要说“我手头有个项目想补…”这种完整交代。
7. 不要伪造已读细节；可以说“我只看到标题/名字忘了/我不确定是不是这个”。
8. 中文为主，可夹少量英文术语。不要用 emoji，也不要用 markdown（不要 **加粗**、代码块、列表符号）；不要空行分段。
9. 像真人闲聊提问，不要下交付式指令（例如“直接告诉我…推荐语”“整理成表格/清单”）；也不要说“你手头那两篇”这种把对方当资料库的口吻。"""
